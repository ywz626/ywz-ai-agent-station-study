...
/**
 * @Author: ywz @CreateTime: 2025-09-21 @Description: 步骤3: 质量检测节点 @Version: 1.0
 */
@Service
@Slf4j
public class Step3QualitySupervisorNode extends AbstractExecuteSupport {
  ...
  protected String doApply(
      ...
      throws Exception {
    AiAgentClientFlowConfigVO qualityClientFlowConfig =
        dynamicContext
    ...
    String supervisionPrompt =
        String.format(
            qualityClientFlowConfig.getStepPrompt(),
  ...
  /** 解析监督结果并上传给前端 */
  private void parseSupervisionResult(
      DefaultExecuteStrategyFactory.DynamicContext dynamicContext,
      String supervisionResult,
      String sessionId) {
  ...
  /** 发送监督结果到流式输出 */
  private void sendSupervisionResult(
      DefaultExecuteStrategyFactory.DynamicContext dynamicContext,
      String supervisionResult,
      String sessionId) {
    AutoAgentExecuteResultEntity result =
        AutoAgentExecuteResultEntity.createSupervisionResult(
            dynamicContext.getStep(), supervisionResult, sessionId);
    sendSseResult(dynamicContext, result);
  }

  /** 发送监督子结果到流式输出（细粒度标识） */
  private void sendSupervisionSubResult(
      DefaultExecuteStrategyFactory.DynamicContext dynamicContext,
      String section,
      String content,
      String sessionId) {
    ...
    if (!content.isEmpty() && !section.isEmpty()) {
      AutoAgentExecuteResultEntity result =
          AutoAgentExecuteResultEntity.createSupervisionSubResult(
