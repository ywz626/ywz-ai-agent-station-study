...
/**
 * @Author: ywz @CreateTime: 2025-09-20 @Description: 顾问角色装配节点 @Version: 1.0
 */
@Service
@Slf4j
public class AiClientAdvisorNode extends AbstractArmorySupport {

  @Resource private AiClientNode aiClientNode;
  ...
  protected String doApply(
      ...
      throws Exception {
    List<AiClientAdvisorVO> advisorVOList =
        dynamicContext.getValue(AiAgentEnumVO.AI_CLIENT_ADVISOR.getDataName());
    ...
    for (AiClientAdvisorVO advisor : advisorVOList) {
      ...
      registerBean(
          AiAgentEnumVO.AI_CLIENT_ADVISOR.getBeanName(advisor.getAdvisorId()),
          Advisor.class,
          advisor1);
    }
    return router(armoryCommandEntity, dynamicContext);
