...
/**
 * @Author: ywz @CreateTime: 2025-09-20 @Description: 客户端装配节点 @Version: 1.0
 */
@Service
@Slf4j
public class AiClientNode extends AbstractArmorySupport {
  /**
   * 执行AI客户端的初始化应用逻辑。该方法会从动态上下文中获取AI客户端配置信息， 并根据配置创建对应的ChatClient实例，注册到Spring容器中。
   *
  ...
  protected String doApply(
      ...
      throws Exception {
    // 获取所有待初始化的 AI 客户端列表
    List<AiClientVO> aiClientVOList =
        dynamicContext.getValue(AiAgentEnumVO.AI_CLIENT.getDataName());
    ...
    // 获取系统提示词映射表
    Map<String, AiClientPromptVO> promptVOList =
    ...
    for (AiClientVO aiClientVO : aiClientVOList) {
      ...
      ChatClient chatClient =
          ChatClient.builder(chatModel)
              ...
              .build();
      registerBean(
          AiAgentEnumVO.AI_CLIENT.getBeanName(aiClientVO.getClientId()),
          ChatClient.class,
          chatClient);
    }
