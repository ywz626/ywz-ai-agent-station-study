...
/**
 * @Author: ywz @CreateTime: 2025-09-20 @Description: 顾问类型枚举 @Version: 1.0
 */
@Getter
@AllArgsConstructor
@NoArgsConstructor
public enum AiClientAdvisorTypeEnumVO {
  ...
  RAG_ANSWER("RagAnswer", "知识库") {
    ...
    public Advisor createAdvisor(AiClientAdvisorVO aiClientAdvisorVO, VectorStore vectorStore) {
      ...
      return new RagAnswerAdvisor(
          vectorStore,
          SearchRequest.builder().topK(topK).filterExpression(filterExpression).build());
    }
  };
  ...
  public abstract Advisor createAdvisor(
      AiClientAdvisorVO aiClientAdvisorVO, VectorStore vectorStore);
}
