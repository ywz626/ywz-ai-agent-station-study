...
public class Step2PlanningNode extends AbstractExecuteSupport {
     ...
     * <p>该方法完成以下主要操作：</p>
     * <ul>
     *   <li>设置当前执行状态为 "PLANNING"</li>
     *   <li>获取用于计划编排的AI客户端配置</li>
     *   <li>构建并优化提示词(prompt)</li>
     *   <li>通过AI客户端生成具体的执行计划</li>
     *   <li>记录和传递中间结果，并推进流程到下一步</li>
     * </ul>
     *
    ...
    protected String doApply(ExecuteCommandEntity executeCommandEntity, DefaultFlowAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        // 获取计划编排所使用的 AI 客户端配置
        AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO = dynamicContext.getClientFlowConfigMap().get(AiClientTypeEnumVO.PLANNING_CLIENT.getCode());
