...
/**
 * @Author: ywz @CreateTime: 2025-09-20 @Description: ToolMcp服务装配 @Version: 1.0
 */
@Service
@Slf4j
public class AiClientToolMcpNode extends AbstractArmorySupport {

  @Resource private AiClientModelNode aiClientModelNode;
  ...
  protected String doApply(
      ...
      throws Exception {
    List<AiClientToolMcpVO> aiClientToolMcpVOList =
        dynamicContext.getValue(AiAgentEnumVO.AI_CLIENT_TOOL_MCP.getDataName());
    ...
    for (AiClientToolMcpVO aiClientToolMcpVO : aiClientToolMcpVOList) {
      ...
      registerBean(
          AiAgentEnumVO.AI_CLIENT_TOOL_MCP.getBeanName(aiClientToolMcpVO.getMcpId()),
          McpSyncClient.class,
          mcpSyncClient);
    }
    return router(armoryCommandEntity, dynamicContext);
    ...
    switch (transportType) {
      ...
      case "sse":
        ...
        HttpClientSseClientTransport sseClientTransport =
            HttpClientSseClientTransport.builder(baseUri).sseEndpoint(sseEndpoint).build();
        McpSyncClient mcpSyncClient =
            McpClient.sync(sseClientTransport)
