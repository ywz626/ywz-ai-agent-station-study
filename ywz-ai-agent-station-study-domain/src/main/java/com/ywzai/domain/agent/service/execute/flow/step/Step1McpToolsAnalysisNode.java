...
/**
 * @Author: ywz @CreateTime: 2025-10-15 @Description: 第一个节点执行MCP工具分析 @Version: 1.0
 */
@Service
@Slf4j
public class Step1McpToolsAnalysisNode extends AbstractExecuteSupport {
  @Resource private Step2PlanningNode step2PlanningNode;

  /**
   * 执行 MCP 工具能力分析阶段的处理逻辑。该方法属于流程中的第一步，用于分析当前可用的 MCP 工具是否能够满足用户请求， 并生成详细的工具能力评估报告，供后续执行阶段参考。
   *
   * <p>此方法不会执行用户的实际请求，仅进行工具能力分析。
   *
  ...
  protected String doApply(
    ...
    AiAgentClientFlowConfigVO aiAgentClientFlowConfigVO =
        dynamicContext.getClientFlowConfigMap().get(AiClientTypeEnumVO.TOOL_MCP_CLIENT.getCode());
    ...
    // 调用 MCP 客户端获取分析结果
    String content = mcpToolsClient.prompt().user(mcpAnalysisPrompt).call().content();
