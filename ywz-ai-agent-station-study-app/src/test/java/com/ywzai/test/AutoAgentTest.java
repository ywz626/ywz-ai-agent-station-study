...
/** AutoAgent 测试类 基于 PlanningAgent 的动态执行机制，实现用户需求的自动化处理 */
@Slf4j
...
public class AutoAgentTest {
  ...
  public void init() {
        ...
        OpenAiChatModel.builder()
            ...
            .defaultOptions(
                OpenAiChatOptions.builder()
                    .model("qwen-max")
                    .toolCallbacks(
                        new SyncMcpToolCallbackProvider(stdioMcpClient(), sseMcpClient())
                            .getToolCallbacks())
                    .build())
            .build();
    ...
    planningChatClient =
        ChatClient.builder(chatModel)
            ...
            .defaultOptions(OpenAiChatOptions.builder().model("qwen-max").maxTokens(2000).build())
            .build();
            ...
            .defaultAdvisors(
                ...
                new RagAnswerAdvisor(
                    ...
                    SearchRequest.builder()
                        ...
                        .build()),
                SimpleLoggerAdvisor.builder().build())
            .defaultOptions(OpenAiChatOptions.builder().model("qwen-max").maxTokens(4000).build())
            .build();
        ...
        ChatClient.builder(chatModel)
            ...
            .defaultAdvisors(
                PromptChatMemoryAdvisor.builder(
                    ...
                    .build(),
                SimpleLoggerAdvisor.builder().build())
            .defaultOptions(OpenAiChatOptions.builder().model("qwen-max").maxTokens(1500).build())
            .build();
  }
  ...
  public void test_planning_agent() {
    ...
    String planningResult =
        planningChatClient
            ...
            .advisors(
                a ->
                    a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, "planning-session-001")
  ...
  public void test_executor_agent() {
    ...
    String executionResult =
        executorChatClient
            ...
            .advisors(
                a ->
                    a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, "executor-session-001")
  ...
  public void test_react_agent() {
    ...
    String reactResult =
        reactChatClient
            ...
            .advisors(
                a ->
                    a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, "react-session-001")
  ...
  public void test_complete_auto_agent_workflow() {
    ...
    String planningResult =
        planningChatClient
            ...
            .advisors(
                a ->
                    a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, "workflow-planning-001")
                        .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 50))
            .call()
            .content();
    ...
    String executionResult =
        executorChatClient
            ...
            .advisors(
                a ->
                    a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, "workflow-execution-001")
                        .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 100))
            .call()
            .content();
    ...
    String summaryResult =
        reactChatClient
            ...
            .advisors(
                a ->
                    a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, "workflow-summary-001")
                        .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 50))
            .call()
            .content();
   ...
   *
   * <p>执行特点： 1. 动态分析用户输入，自主决定执行策略 2. 根据每轮执行结果，智能判断下一步行动 3. 支持可配置的最大执行步数 4. 具备任务完成判断和提前终止机制 5. 模拟真实的
   * Agent 思考-行动-观察循环
   */
  @Test
  public void test_dynamic_multi_step_execution() {
    ...
    ChatClient precisionExecutorClient =
        ChatClient.builder(chatModel)
            .defaultSystem(
                """
                        # 角色
            ...
            .defaultAdvisors(
                PromptChatMemoryAdvisor.builder(
                        MessageWindowChatMemory.builder().maxMessages(150).build())
                    .build(),
                new RagAnswerAdvisor(
                    vectorStore,
                    SearchRequest.builder()
    ...
    ChatClient qualitySupervisorClient =
        ChatClient.builder(chatModel)
            .defaultSystem(
                """
                        # 角色
            ...
            .defaultAdvisors(
                PromptChatMemoryAdvisor.builder(
                        MessageWindowChatMemory.builder().maxMessages(80).build())
                    .build(),
                SimpleLoggerAdvisor.builder().build())
            .defaultOptions(
                OpenAiChatOptions.builder()
      ...
      try {
        ...
        String analysisResult =
            taskAnalyzerClient
                ...
                .advisors(
                    a ->
                        a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, sessionId + "-analyzer")
                            .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 100))
                .call()
                .content();
        ...
        String executionResult =
            precisionExecutorClient
                ...
                .advisors(
                    a ->
                        a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, sessionId + "-executor")
        ...
        String supervisionResult =
            qualitySupervisorClient
                .prompt(supervisionPrompt)
                .advisors(
                    a ->
                        a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, sessionId + "-supervisor")
    ...
    if (!isCompleted) {
      ...
      String summaryResult =
          reactChatClient
              ...
              .advisors(
                  a ->
                      a.param(CHAT_MEMORY_CONVERSATION_ID_KEY, sessionId + "-summary")
                          .param(CHAT_MEMORY_RETRIEVE_SIZE_KEY, 50))
              .call()
              .content();
  ...
  /** 提取下一步任务 */
  private String extractNextTask(
      String analysisResult, String executionResult, String currentTask) {
    // 从分析结果中提取下一步策略
  ...
  private void logExecutionSummary(
      int maxSteps, StringBuilder executionHistory, boolean isCompleted) {
    log.info("\n📊 === 动态多轮执行总结 ====");
    ...
    var stdioParams =
        ServerParameters.builder("npx.cmd")
            .args(
                ...
                "C:\\Users\\ywz\\Desktop",
                "D:\\java\\ideapublic\\ywz-ai-agent-station-study\\ywz-ai-agent-station-study-app")
            .build();
