...
/**
 * @Author: ywz @CreateTime: 2025-10-16 @Description: 定时任务配置 @Version: 1.0
 */
@Configuration
@EnableScheduling
...
public class TaskJobAutoConfigBean {
  ...
  public TaskScheduler taskScheduler(TaskJobAutoProperties properties) {
    ...
    log.info(
        "xfg-wrench，任务调度器初始化完成。线程池大小: {}, 线程名前缀: {}",
  ...
  public ITaskJobService taskJobService(
      TaskScheduler xfgWrenchTaskScheduler, List<ITaskDataProvider> taskDataProviders) {
  ...
  public TaskJob taskJob(TaskJobAutoProperties properties, ITaskJobService taskJobService) {
    log.info(
        "xfg-wrench，任务调度作业初始化完成。刷新间隔: {}ms, 清理cron: {}",
        properties.getRefreshInterval(),
        properties.getCleanInvalidTasksCron());
