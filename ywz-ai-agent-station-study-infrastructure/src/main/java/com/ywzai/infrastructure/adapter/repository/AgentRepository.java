...
/**
 * @Author: ywz @CreateTime: 2025-09-19 @Description: Agent仓储服务实现类 @Version: 1.0
 */
@Repository
@Slf4j
public class AgentRepository implements IAgentRepository {
  ...
  @Resource private IAiAgentTaskScheduleDao aiAgentTaskScheduleDao;
    ...
    return aiAgentFlowConfigs.stream()
        .collect(
            Collectors.toMap(
                ...
                config ->
                    new AiAgentClientFlowConfigVO( // value: 转换成 AiAgentClientFlowConfigVO
                        config.getClientId(),
  ...
  public List<AiAgentTaskScheduleVO> queryAllActivityTaskSchedule() {
    ...
    for (AiAgentTaskSchedule aiAgentTaskSchedule : aiAgentTaskSchedules) {
      aiAgentTaskScheduleVOS.add(
          new AiAgentTaskScheduleVO(
   ...
   *
   * @param clientIds 客户端ID列表，用于查询对应的客户端信息及其相关配置。 如果该参数为null或空列表，则直接返回空列表。
   * @return 返回与输入客户端ID对应的有效AiClientVO对象列表。 每个AiClientVO包含客户端基本信息以及关联的模型、提示词、工具和顾问ID列表。
   */
  @Override
    ...
    for (String clientId : clientIds) {
      ...
      for (String advisorId : advisorIds) {
        ...
        if (advisor != null && advisor.getStatus() == 1) {
          ...
          AiClientAdvisorVO advisorVO =
              AiClientAdvisorVO.builder()
    ...
    for (String clientId : clientIds) {
      ...
      for (String promptId : promptIds) {
        ...
        AiClientSystemPrompt aiClientSystemPrompt =
            aiClientSystemPromptDao.queryByPromptId(promptId);
        if (aiClientSystemPrompt != null && aiClientSystemPrompt.getStatus() == 1) {
          // 构建AiClientPromptVO对象
          AiClientPromptVO aiClientPromptVO =
              AiClientPromptVO.builder()
  ...
  public List<AiClientModelVO> getAiClientModelVOListByClientIds(List<String> clientIds) {
    ...
    for (String clientId : clientIds) {
      ...
      if (model != null && model.getStatus() == 1) {
        ...
        AiClientModelVO aiClientModelVO =
  ...
  @Resource private IAiClientRagOrderDao aiClientRagOrderDao;
